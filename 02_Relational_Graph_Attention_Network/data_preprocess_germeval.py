...
def parse_args():
    ...
    # Required parameters
    parser.add_argument('--data_path', type=str, default='./data/studycheck',
...
def text2docs(file_path, predictor):
    ...
    for i in tqdm(range(len(sentences))):
        #print(sentences[i])
        sent_pred = predictor.predict(sentence=sentences[i])
        if len(sent_pred) > 512:
            print(len(sent_pred))
        docs.append(sent_pred)
    return docs
        ...
        for sentence in root:
            ...
            for c in terms:
                if c.attrib['polarity'] == 'conflict':
                    continue
                if '.' in str(c.attrib['target']) or '-' in str(c.attrib['target']):
                    continue
                # index in strings, we want index in tokens
                left_index = int(c.attrib['from'])
                right_index = int(c.attrib['to'])
                
                target = sentence.find('text').text[left_index:right_index].replace('.', ' ')
                example["aspect_sentiment"].append((target, c.attrib['polarity']))
                #print(example['aspect_sentiment'])
                
                example_ohne_at = re.sub(r'[;@)(:/#$%&]', ' ', example['sentence'])
                left_word_offset = len(tk.tokenize(example_ohne_at[:left_index]))
...
def main():
    ...
    train_file='StudyCheck_Train_Clean.xml'
    test_file='StudyCheck_Test_Clean.xml'
